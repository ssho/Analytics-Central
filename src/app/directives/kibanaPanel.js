...
function (angular) {
  ...
  angular
    ...
    .directive('kibanaPanel', function($compile) {
      var containerOriginal = '<div _Id_ class="panelCont"></div>';
      var container = '<div _Id_ class="panelCont"></div>';
      ...
      var editorTemplate =
        ...
        '<div class="row-fluid panel-extra"><div class="panel-extra-container">' +
          ...
          '<span class="extra row-button" ng-show="panel.editable != false">' +
            ...
            '<i class="icon-remove pointer" data-placement="bottom" bs-tooltip="\'Remove\'"></i></span>'+
          '</span>' +

          '<span class="row-button extra" ng-show="panel.editable != false">' +
            '<span bs-modal="\'app/partials/paneleditor.html\'" class="pointer">'+
            '<i class="icon-cog pointer" data-placement="bottom" bs-tooltip="\'Configure\'"></i></span>'+
          '</span>' +
          ...
          '<span ng-repeat="task in panelMeta.modals" class="row-button extra" ng-show="panel.spyable">' +
            '<span bs-modal="task.partial"class="pointer"><i ' +
              'bs-tooltip="task.description" data-placement="bottom" ng-class="task.icon" class="pointer"></i></span>'+
          '</span>' +
          ...
          '<span class="dropdown row-button extra" bs-tooltip="\'Export\'" data-placement="bottom" ng-show="panelMeta.exportfile">' +
            '<span class="pointer" class="dropdown-toggle" data-toggle="dropdown">' +
                '<i class="icon-save" data-placement="bottom" class="pointer"></i>' +
            '</span>' +
          ...
          '<span class="dropdown row-button extra" bs-tooltip="\'Export\'" data-placement="bottom" ng-show="panelMeta.exportfiletocsv">' +
            '<span class="pointer" class="dropdown-toggle" data-toggle="dropdown">' +
              '<a ng-click="exportfiletocsv()"><i class="icon-save" class="pointer"></i></a>' +
            '</span>' +          
          '</span>' +      
...
//          '</span>' +
          ...
          //'<span class="extra row-button">' +
          //  '<span class="pointer" >' +
          //     '<i class="icon-print pointer" onclick="printDiv(\'_printDivId_\')" data-placement="left" bs-tooltip="\'print this graph\'" ></i>' +
          //'</span>'+
          //'</span>' +
      ...
      var editorTemplateOriginal = editorTemplate;  
      return {
        restrict: 'E',
        link: function($scope, elem, attr) {
          ...
          function loadModule($module,inRandomNumber) {
            $module.appendTo(elem);
            container = containerOriginal;
            container = container.replace('_Id_ ', 'id="printId' + inRandomNumber + '" ' );
            elem.wrap(container);
            /* jshint indent:false */
          ...
          $scope.$watch(attr.type, function (name) {
            ...
            $scope.require([
              ...
              if ($controllers.length) {
                editorTemplate = editorTemplateOriginal;
                editorTemplate = editorTemplate.replace('_printDivId_','printId'+randomNumber);
                $controllers.first().prepend(editorTemplate);
                $scope.require([
                  'panels/'+name+'/module'
                ], function() {
                  loadModule($module,randomNumber);
                });
              } else {
                loadModule($module,randomNumber);
              }
