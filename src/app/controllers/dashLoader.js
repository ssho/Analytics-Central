...
function (angular, _) {
  ...
  module.controller('dashLoader', function($scope, $http, timer, dashboard, alertSrv) {
    ...
    $scope.init = function() {
      ...
      $scope.get_user_default_dashboard();
    };
    ...
    $scope.showDropdown = function(type) {
      ...
      if(type === 'load') {
        //return (_l.load_elasticsearch || _l.load_gist || _l.load_local);
        return true;  // in order to show load menu
      }
      if(type === 'save') {
        //return (_l.save_elasticsearch || _l.save_gist || _l.save_local || _l.save_default);
        return true;  // in order to show save menu
      }
      if(type === 'share') {
        return (_l.save_temp);
      }
    ...
    $scope.get_dashboard_list = function(){
       if (dashboard.get_dashboard_list()){

       } else {
          alertSrv.set('Cannot load dashboard list!','','error',5000);
       }
    };

    $scope.get_user_default_dashboard = function(){
       if (dashboard.get_user_default_dashboard()){

       } else {
          alertSrv.set('Cannot load default dashboard.','','error',5000);
       }
    };

    $scope.load_dashboard = function(dashboardId){
       if (dashboard.load_dashboard(dashboardId)){

       } else {
          alertSrv.set('Cannot load dashboard.','','error',5000);
       }

    };

    $scope.save_dashboard_to_server = function(inputDashboardName) {
      if (dashboard.save_dashboard_to_server(inputDashboardName)){
          dashboard.get_dashboard_list();
      } else {
         alertSrv.set('Cannot save dashboard.','','error',5000);
      }
    };
