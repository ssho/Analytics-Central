...
function($, _) {
  ...
  kbn.download_response_to_csv = function(response,basename){
    var blob;
    blob = new Blob([response.toString()], {type: "text/csv;charset=utf-8"});
    window.saveAs(blob, basename +"-"+new Date().getTime()+".csv");
    return true;
  }   
  ...
  kbn.top_field_values = function(docs,field,count,grouped) {
   ...
   /**
     ...
     * @param  {Integer,optional} user_interval  User-specified history interval (defaults to 0)
     * @return {Number}
     *
     */
  ...
  kbn.round_gap = function(interval) {
   ...
   /**
     ...
     * @param  {number} user_interval User-specified history interval
     * @return {number}
     *
     */
