...
function (angular, _, config, moment) {
  ...
  // Add solr support to query indices
  ...
  module.service('kbnIndex', function($http, alertSrv) {
    ...
    this.collections = function(solr_server,rootscopeToken) {
      //if (rootscopeToken.length > 0 ){
         extraToken = {"Authorization":"Bearer " + rootscopeToken};
         //console.debug(JSON.stringify(extraToken));
      //}else{
      //   extraToken = {};
      //};
    ...
    function all_collections(solr_server) {
      ...
      var  something = $http({
        ...
        // Need to test this with SolrCloud and LWS
    ...
    function all_indices() {
      var something = $http({
        ...
        // Solr has no concept of indices, instead it uses Core to store data.
