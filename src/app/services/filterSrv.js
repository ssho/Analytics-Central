define([
  ...
  'moment',
  'kbn'
], function (angular, _, config,moment,kbn) {
  'use strict';
  ...
  module.service('filterSrv', function(dashboard, ejsResource, sjsResource) {
    ...
    this.init = function() {
      ...
      _.each(self.getByType('time',true),function(time) {
         //console.log("filter srv: time.id:"+time.id);
        self.list[time.id].from = time.from;
        self.list[time.id].to = time.to;
        //if (dashboard.current.loader.user_selected_time_mode == "relative" || dashboard.current.loader.user_selected_time_mode == "since" ){
        if (dashboard.current.loader.user_selected_time_mode == "relative"  ){
           //fix the bug, when relative date mode load from db, re-gen the fromDateObj
           if (dashboard.current.loader.user_selected_time_option === "") dashboard.current.loader.user_selected_time_option = "30d";
           self.list[time.id].fromDateObj = new Date(moment(kbn.time_ago(dashboard.current.loader.user_selected_time_option)));
           self.list[time.id].toDateObj = new Date(moment());
        }else{
    ...
    this.replaceSolrCollectionTimefieldByVendor = function(vendor,fqString) {
      var fqIndex = fqString.indexOf("fq=");
      var colonIndex = fqString.indexOf(":");
      var timefieldString ="";
      //console.log("original fq string " + fqString);
      if (fqIndex > 0 && colonIndex > 0){
         timefieldString = eval("config." + vendor + "_timefield");
         fqString = fqString.substring(0,4) + timefieldString + fqString.substring(colonIndex);
         //console.log("------------------new fq time :" + fqString);
      };
      return fqString;
    };
