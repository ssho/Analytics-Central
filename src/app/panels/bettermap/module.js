...
define([
    ...
    function (angular, app, _, L, localRequire) {
        ...
        module.controller('bettermap', function ($scope, querySrv, dashboard, filterSrv, $rootScope) {
            $scope.panelMeta = {
                modals: [
                    {
            ...
            $scope.get_data = function (segment, query_id) {
                $scope.require(['./leaflet/plugins'], function () {
                    ...
                    request = request.query(
                        $scope.sjs.FilteredQuery(boolQuery, filterSrv.getBoolFilter(filterSrv.ids)))
                    ...
                    // FIXED LatLong Query
                    if ($scope.panel.lat_start && $scope.panel.lat_end && $scope.panel.lon_start
                        && $scope.panel.lon_end && $scope.panel.field) {
                        fq += '&fq=' + $scope.panel.field + ':[' + $scope.panel.lat_start + ','
                            + $scope.panel.lon_start + ' TO ' + $scope.panel.lat_end + ',' + $scope.panel.lon_end + ']';
                    }
