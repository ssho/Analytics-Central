...
function (angular, app, _, moment, kbn, $) {
  ...
  module.controller('timepicker', function($scope, $rootScope, $timeout, timer, $http, dashboard, filterSrv) {
    ...
    var _d = {
      ...
      info_description: 'Select a time range for data sets.'
    };
      ...
      $scope.$on('refresh', function() {
        if(filterSrv.idsByType('time').length > 0) {
          ...
          if($scope.time.from.diff(moment.utc(time.from),'seconds') !== 0 ||
            ...
            $scope.set_mode(dashboard.current.loader.user_selected_time_mode);
            // These 3 statements basicly do everything time_apply() does
    ...
    $scope.set_timespan = function(timespan) {
      ...
      dashboard.current.loader.user_selected_time_mode  =  $scope.panel.mode;
      dashboard.current.loader.user_selected_time_option = timespan;
    ...
    function compile_time(time) {
      ...
      if ($scope.panel.mode === 'relative') {
        ...
        if (timeUnit === "YEAR"){
          // fix the year bug, should be from 1 year ago to today
          filterTime.from = 'NOW/' + 'DAY' + '-' + timeNumber + timeUnit;
          filterTime.to   = 'NOW/' + 'DAY' + '%2B1' + 'DAY'; 
        } ;
        // Add Date objects representation of from and to, for use with history panel
